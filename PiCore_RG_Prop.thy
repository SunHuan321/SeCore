  ...
  proof -
      ...
      using conjoin_comm_imp_rely by (metis (mono_tags, lifting)) 
    {
      ...
      from a2 have "\<forall>ef\<in>all_evts_esspec (evtsys_spec (rgf_EvtSeq ef esf)). is_basicevt ef" 
        using evtsys_spec_evtseq[of ef esf] all_evts_same[of "rgf_EvtSeq ef esf"]
          by (metis DomainE E\<^sub>e_def prod.sel(1)) 
      with p1 p2 a6 a2 a3 a4 a8 have "\<exists>ie. ie < i \<and> (\<exists>e. (cs k)!ie -es-(EvtEnt e\<sharp>k)\<rightarrow> (cs k)!(Suc ie))
              \<and> (\<forall>j. j > ie \<and> j < i \<longrightarrow> \<not>(\<exists>e. (cs k)!j -es-(EvtEnt e\<sharp>k)\<rightarrow> (cs k)!(Suc j)))"
        using cmd_impl_evtent_before_and_cmds[of c cs k "evtsys_spec (rgf_EvtSeq ef esf)" s x] 
          cpts_of_es_def[of "EvtSeq e es" s x] by auto
      then obtain ie and ev where c4: "ie < i \<and> ((cs k)!ie -es-(EvtEnt ev\<sharp>k)\<rightarrow> (cs k)!(Suc ie))
              \<and> (\<forall>j. j > ie \<and> j < i \<longrightarrow> \<not>(\<exists>e. (cs k)!j -es-(EvtEnt e\<sharp>k)\<rightarrow> (cs k)!(Suc j)))" by auto
      with p1 p6 a3 have "\<forall>m. m > ie \<and> m \<le> i \<longrightarrow> getx_es ((cs k)!m) k = ev"
        using evtent_impl_curevt_in_cpts_es2[of c cs ie k ev i] by auto
      ...
      from a3 c4 have c8: "ie < i \<and> (?eslh!ie -es-(EvtEnt ev\<sharp>k)\<rightarrow> ?eslh!(Suc ie))
              \<and> (\<forall>j. j > ie \<and> j < i \<longrightarrow> \<not>(\<exists>e. ?eslh!j -es-(EvtEnt e\<sharp>k)\<rightarrow> ?eslh!(Suc j)))" by force
      from a3 a81 a82 a83 c8 c0 have "\<forall>i. i \<le> ie \<longrightarrow> getspc_es (?eslh ! i) = EvtSeq e es"
        using evtseq_evtent_befaft[of ?eslh e es s x esl2 ie]
      ...
      have "(gets_es (cs k ! i), gets_es (cs k ! Suc i)) \<in> Guar\<^sub>f (the (evtrgfs (getx_es (cs k ! i) k)))"
        proof(cases "Suc i < ?n - 1")
          ...
          with c1 c2 e0 have e4: "\<exists>t. (el ! i) -et-t\<rightarrow> (el ! Suc i)"
            using cpts_of_ev_def[of e s x] notran_confeqi[of el i]
        ...
        next
          ...
          ultimately have "(sn1,sn)\<in>Guar\<^sub>e ef" using d42 gets_e_def[of "(getspc_e (last el), sn1, xn1)"]
            gets_e_def[of "(AnonyEvent None, sn, xn)"] by (metis fst_conv snd_conv) 
          moreover
      ...
      then have "(gets_es (cs k ! i), gets_es (cs k ! Suc i)) \<in> Guar\<^sub>f (the (evtrgfs (getx_es (cs k ! i) k)))"
        proof(cases "\<forall>i. Suc i \<le> length ?esl \<longrightarrow> getspc_es (?esl ! i) \<noteq> es")
          ...
          with p0 p1 p8 p2 p9 p10 p4 p6 p16 show ?thesis 
            using act_cpts_evtseq_sat_guar_curevt_fstseg_new2[of ef esf prea 
        ...
        next
          ...
          with a8 have c4: "n \<noteq> 0" using getspc_es_def[of "cs k ! 0"] 
            by (metis (no_types, hide_lams) add.commute add.right_neutral fst_conv
          ...
          let ?c12 = "take (Suc n) c"
            ...
            with p15 c4 have d2: "?c12\<in>cpts_of_pes pes s x" 
              using cpts_of_pes_def[of pes s x]
            ...
            from p2 c1 c3 p11 have "\<forall>k. (?cs12 k) \<in> cpts_of_es (pes k) s x"
              proof -
              {
                ...
                moreover 
                with c1 c3 p11 have "(?cs12 k')\<in>cpts_es" using cpts_es_take[of "cs k'" "n"]
            ...
            have r1[rule_format]: "(\<forall>i. Suc i < length (?cs12 k) \<and> ((?cs12 k ! i) -es-(Cmd cmd)\<sharp>k\<rightarrow> (?cs12 k ! Suc i)) \<longrightarrow>
              ...
              using act_cpts_evtseq_sat_guar_curevt_fstseg_new2_withlst_pst[of ef esf prea 
          ...
          next
            ...
            from p15 p11 c1 c3 c4 c5_1 have "?c2\<in>cpts_pes" using cpts_pes_dropi[of c "n-1"]
            ...
            have "\<forall>n k. n \<le> length (cs k) \<and> n > 0 
                                    \<longrightarrow> take n (cs k)\<in>assume_es(Pre k, Rely k)"
  ...
  next
  {
...

    {
      ...
      with a9 b0 b1 b2 b3 b4 b5 b6 b7 b8 b9 b10
        have "\<forall>i. Suc i < length (cs k) \<and> cs k ! i -es-Cmd cmd\<sharp>k\<rightarrow> cs k ! Suc i \<longrightarrow>
               (gets_es (cs k ! i), gets_es (cs k ! Suc i)) \<in> Guar\<^sub>f (the (evtrgfs (getx_es (cs k ! i) k)))"
        using act_cpts_evtsys_sat_guar_curevt_gen0_new2[of "rgf_EvtSys esf" prea 
    ...
    show "s0 \<in> prea \<longrightarrow>
      ...
      proof -
      {
...

        {
          ...
          ultimately
            have "(gets_es ((cs k)!i), gets_es ((cs k)!(Suc i)))\<in>Guar\<^sub>f (the (evtrgfs (getx_es ((cs k)!i) k)))"
            using act_cpts_es_sat_guar_curevt_new2[of "fst (pesfa k)" "Pre\<^sub>e\<^sub>s (pesfa k)" 
