...
inductive rghoare_p :: "['s prog, 's set, ('s \<times> 's) set, ('s \<times> 's) set, 's set] \<Rightarrow> bool"
...
where
  Basic: "\<lbrakk> pre \<subseteq> {s. f s \<in> post}; {(s,t). s \<in> pre \<and> (t=f s)} \<subseteq> guar;
           ...
           \<Longrightarrow> \<turnstile> Basic f sat\<^sub>p [pre, rely, guar, post]"

| Seq: "\<lbrakk> \<turnstile> P sat\<^sub>p [pre, rely, guar, mid]; \<turnstile> Q sat\<^sub>p [mid, rely, guar, post] \<rbrakk>
           \<Longrightarrow> \<turnstile> Seq P Q sat\<^sub>p [pre, rely, guar, post]"

| Cond: "\<lbrakk> stable pre rely; \<turnstile> P1 sat\<^sub>p [pre \<inter> b, rely, guar, post];
           \<turnstile> P2 sat\<^sub>p [pre \<inter> -b, rely, guar, post]; \<forall>s. (s,s)\<in>guar \<rbrakk>
          \<Longrightarrow> \<turnstile> Cond b P1 P2 sat\<^sub>p [pre, rely, guar, post]"
...
| While: "\<lbrakk> stable pre rely; (pre \<inter> -b) \<subseteq> post; stable post rely;
          ...
          \<Longrightarrow> \<turnstile> While b P sat\<^sub>p [pre, rely, guar, post]"
...
| Await: "\<lbrakk> stable pre rely; stable post rely;
           ...
           \<Longrightarrow> \<turnstile> Await b P sat\<^sub>p [pre, rely, guar, post]"

| Nondt: "\<lbrakk> pre \<subseteq> {s. (\<forall>t. (s,t) \<in> r \<longrightarrow> t \<in> post) \<and> (\<exists>t. (s,t) \<in> r)}; {(s,t). s \<in> pre \<and> (s,t)\<in>r} \<subseteq> guar;
            stable pre rely; stable post rely \<rbrakk>
           \<Longrightarrow> \<turnstile> Nondt r sat\<^sub>p [pre, rely, guar, post]"
...
lemma ctran_in_comm:
  "\<lbrakk>(s, s) \<in> guar; (Q, s) # xs \<in> commit_p(guar, post)\<rbrakk>
  \<Longrightarrow> (P, s) # (Q, s) # xs \<in> commit_p(guar, post)"
apply(simp add:commit_p_def)
apply(simp add:getspc_p_def gets_p_def)
apply clarify
apply(case_tac i,fastforce+)
...
lemma unique_ctran_Basic [rule_format]:
  "\<forall>s i. x \<in> cpts_p \<longrightarrow> x ! 0 = (Some (Basic f), s) \<longrightarrow>
...
apply(ind_cases "((Some (Basic f), sa), Q, t) \<in> ptran" for sa Q t)
apply simp
...
lemma exists_ctran_Basic_None [rule_format]:
  "\<forall>s i. x \<in> cpts_p \<longrightarrow> x ! 0 = (Some (Basic f), s)
...
lemma unique_ctran_Await [rule_format]:
  "\<forall>s i. x \<in> cpts_p \<longrightarrow> x ! 0 = (Some (Await b c), s) \<longrightarrow>
  Suc i<length x \<longrightarrow> x!i -c\<rightarrow> x!Suc i \<longrightarrow>
  (\<forall>j. Suc j<length x \<longrightarrow> i\<noteq>j \<longrightarrow> x!j -pe\<rightarrow> x!Suc j)"
apply(induct x,simp+)
apply clarify
...
apply(case_tac i)
 ...
 apply(erule ptran.cases,simp_all)
 apply(force elim: not_ctran_None)
apply(ind_cases "((Some (Await b c), sa), Q, t) \<in> ptran" for sa Q t,simp)
apply(drule_tac i=nat in not_ctran_None,simp)
apply(erule petranE,simp)
done

lemma exists_ctran_Await_None [rule_format]:
  "\<forall>s i.  x \<in> cpts_p \<longrightarrow> x ! 0 = (Some (Await b c), s)
  \<longrightarrow> i<length x \<longrightarrow> fst(x!i)=None \<longrightarrow> (\<exists>j<i. x!j -c\<rightarrow> x!Suc j)"
apply(induct x,simp+)
apply clarify
apply(erule cpts_p.cases,simp)
...
lemma Cond_sound:
  "\<lbrakk> stable pre rely; \<Turnstile> P1 sat\<^sub>p [pre \<inter> b, rely, guar, post];
  \<Turnstile> P2 sat\<^sub>p [pre \<inter> - b, rely, guar, post]; \<forall>s. (s,s)\<in>guar\<rbrakk>
  \<Longrightarrow> \<Turnstile> (Cond b P1 P2) sat\<^sub>p [pre, rely, guar, post]"
apply(unfold prog_validity_def)
apply clarify
...
lemma Seq_sound1 [rule_format]:
  "x\<in> cpt_p_mod \<Longrightarrow> \<forall>s P. x !0=(Some (Seq P Q), s) \<longrightarrow>
...
apply simp_all
  ...
  apply(ind_cases "((Some (Seq Pa Q), sa), None, t) \<in> ptran" for Pa sa t)
...
lemma Seq_sound2 [rule_format]:
  "x \<in> cpts_p \<Longrightarrow> \<forall>s P i. x!0=(Some (Seq P Q), s) \<longrightarrow> i<length x
...
apply(ind_cases "((Some (Seq Pa Q), sa), t) \<in> ptran" for Pa sa t)
 apply simp
...
lemma assum_after_body:
  ...
  (Some (While b P), s) # (Some (Seq P (While b P)), s) #
   map (lift (While b P)) xs @ ys \<in> assume_p (pre, rely)\<rbrakk>
  \<Longrightarrow> (Some (While b P), snd (last ((Some P, s) # xs))) # ys \<in> assume_p (pre, rely)"
...
lemma While_sound_aux [rule_format]:
  ...
  \<Longrightarrow>  \<forall>s xs. x=(Some(While b P),s)#xs \<longrightarrow> x\<in>assume_p(pre, rely) \<longrightarrow> x \<in> commit_p (guar, post)"
apply(erule cpt_p_mod.induct)
apply safe
...
apply(subgoal_tac "\<forall>i. Suc i < length ((None, t) # xs) \<longrightarrow> (((None, t) # xs) ! i, ((None, t) # xs) ! Suc i)\<in> petran")
 prefer 2
 apply clarify
...
lemma unique_ctran_Nondt [rule_format]:
  "\<forall>s i. x \<in> cpts_p \<longrightarrow> x ! 0 = (Some (Nondt r), s) \<longrightarrow>
  Suc i<length x \<longrightarrow> x!i -c\<rightarrow> x!Suc i \<longrightarrow>
  (\<forall>j. Suc j<length x \<longrightarrow> i\<noteq>j \<longrightarrow> x!j -pe\<rightarrow> x!Suc j)"
apply(induct x,simp)
apply simp
...
apply(case_tac i)
 ...
 apply(erule ptran.cases,simp_all)
 apply(force elim: not_ctran_None)
apply(ind_cases "((Some (Nondt r), sa), Q, t) \<in> ptran" for sa Q t)
apply simp
apply(drule_tac i=nat in not_ctran_None,simp)
apply(erule petranE,simp)
done
...
lemma exists_ctran_Nondt_None [rule_format]:
  "\<forall>s i. x \<in> cpts_p \<longrightarrow> x ! 0 = (Some (Nondt r), s)
  \<longrightarrow> i<length x \<longrightarrow> fst(x!i)=None \<longrightarrow> (\<exists>j<i. x!j -c\<rightarrow> x!Suc j)"
apply(induct x,simp)
apply simp
apply clarify
...
lemma Nondt_sound:
  "\<lbrakk>pre \<subseteq> {s. (\<forall>t. (s,t) \<in> r \<longrightarrow> t \<in> post) \<and> (\<exists>t. (s,t) \<in> r)}; {(s,t). s \<in> pre \<and> (s,t)\<in>r} \<subseteq> guar;
            stable pre rely; stable post rely\<rbrakk>
  \<Longrightarrow> \<Turnstile> Nondt r sat\<^sub>p [pre, rely, guar, post]"
apply(unfold prog_validity_def)
apply(clarify)
...
apply(rule conjI)
 ...
 apply(case_tac "x!i")
 apply clarify
 apply(drule_tac s="Some (Nondt r)" in sym,simp)
 apply(thin_tac "\<forall>j. H j" for H)
 apply(force elim:ptran.cases)
apply(simp add:cpts_of_p_def)
apply clarify

apply(frule_tac i="length x - 1" and r=r in exists_ctran_Nondt_None,simp+)
  apply(case_tac x,simp+)
  apply(rule last_fst_esp,simp add:last_length)
 apply (case_tac x,simp+)
apply(simp add:assume_p_def gets_p_def)
          ...
          proof -
          {
            ...
            have "esl\<in> commit_es (guar, post)"
              proof(cases "\<forall>i. Suc i \<le> length esl \<longrightarrow> getspc_es (esl ! i) \<noteq> es")
                ...
                with p4 have b2: "\<forall>i. Suc i<length el \<longrightarrow> (\<exists>t. el!i -et-t\<rightarrow> el!(Suc i)) 
