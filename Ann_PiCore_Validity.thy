...
definition ann_preserves_p :: "'s pconf \<Rightarrow> bool" where
  "ann_preserves_p conf \<equiv> (gets_p conf) \<in> (ann_pre_p (getspc_p conf))"
...
definition pes_validity :: "('l,'k,'s) paresys \<Rightarrow> 's set \<Rightarrow> ('s \<times> 's) set \<Rightarrow> ('s \<times> 's) set \<Rightarrow> 's set \<Rightarrow> bool" 
  ...
  "\<Turnstile> pes SAT [pre, rely, guar, post] \<equiv> 
   \<forall>s x. (cpts_of_pes pes s x) \<inter> assume_pes(pre, rely) \<subseteq> commit_pes(guar, post)"

subsection \<open>Lemmas of Correctness Formulas\<close>

lemma ann_preserves_p_cmd: " b \<in> ann_pre P \<Longrightarrow> ann_preserves_p (Some P, b)"
  by (simp add: ann_preserves_p_def gets_p_def getspc_p_def)

lemma ann_preserves_p_None: " fst conf = None \<Longrightarrow> ann_preserves_p conf"
  by (simp add: ann_preserves_p_def gets_p_def getspc_p_def)
